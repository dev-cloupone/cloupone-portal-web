...
export function CcEmailsInput({ value, onChange, disabled, maxEmails = 10 }: CcEmailsInputProps) {
  ...
  return (
    <div className="relative">
      <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3.5 py-2 focus-within:border-accent focus-within:ring-1 focus-within:ring-accent min-h-[42px]">
        {value.map((email) => (
          <span
            key={email}
        ...
        {value.length > 0 && (
          <span className="ml-auto text-[11px] text-text-muted shrink-0">{value.length}/{maxEmails}</span>
        )}
      </div>
